...
template<class Ele, class Keytype> 
void 
hash<Ele,Keytype>::merge(hash* thread_hash) {
    for (unsigned i = 0; i < my_size; ++i) {
        list<Ele, Keytype>* cur_list = &(thread_hash->entries[i]);
        // Pop the head from the list ultil it is empty
        Ele* cur = cur_list->pop();
        while (cur != NULL) {
          Ele* s = entries[i].lookup(cur->key());
          if (!s) {
            entries[i].push(cur);
          } else {
            s->count += cur->count;
